...
def envoyer_alerte_sms(maladie, culture, severite, surface, parcelle_id=None):
    """
    Envoie un SMS d'alerte quand le niveau BBCH est critique.

    Args:
        maladie     (str)   : nom de la maladie détectée
        culture     (str)   : culture affectée
        severite    (int)   : niveau BBCH (0-4)
        surface     (float) : % surface foliaire atteinte
        parcelle_id (int)   : identifiant de la parcelle (optionnel)
    """
